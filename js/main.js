...
modelsJson.map((item, index)=>{
    ...
    modelsItem.querySelector('.models-item--disc').innerHTML = item.description;
    ...
    modelsItem.querySelector('a').addEventListener('click', (e)=>{
        ...
        c('.modelsInfo--disc').innerHTML = modelsJson[key].description;
        ...
        c('.modelsInfo--amt').innerHTML = modalAmt;
...
c('.modelsInfo--amtless').addEventListener('click', ()=>{
    if(modalAmt > 1) {
        modalAmt--;
        c('.modelsInfo--amt').innerHTML = modalAmt;
    }
...
c('.modelsInfo--amtmore').addEventListener('click', ()=>{
    modalAmt++;
    c('.modelsInfo--amt').innerHTML = modalAmt;
});
...
function updateCart() {
    ...
    if(cart.length > 0) {
        ...
        cart.map((itemCart, index)=>{
            ...
            cartItem.querySelector('.cart--item--amt').innerHTML = itemCart.qt;
            cartItem.querySelector('.cart--item-amtless').addEventListener('click',()=>{
                if(itemCart.qt > 1) {
                    itemCart.qt--
                } else {
                    cart.splice(index, 1);
